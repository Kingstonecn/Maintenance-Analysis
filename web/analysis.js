...
reg(38,4,3,'设备健康度评分','L,AK,AQ','综合频率/时长/成本给设备健康打分',execEquipHealth,'看健康度评分最高的设备，这些是优先关注和投入资源的目标。用于设备分级管理。');
...
reg(41,5,2,'维修人效率对比','X,AK','各维修人维修时长分布箱线对比',execRepairerCompare,'看各维修人平均时长分布，偏长的需技能培训。用于制定维修人培训计划。');
...
reg(45,5,3,'维修阶段时长分解','Y,AA,AF,AG','申请到确认到维修各阶段时长占比',execPhaseBreakdown,'看哪个阶段占比最大，占比大的阶段是流程优化重点。用于缩短整体维修周期。');
...
reg(49,6,2,'产能影响量化','AR,AT,AU','影响产能维修单的时长与数量分析',execCapacityImpact,'看影响产能的维修单数和时长，量化产能损失。用于产能保障优先级排序。');
reg(50,6,2,'质量影响分析','AS','影响质量的维修单数量与特征',execQualityImpact,'看影响质量的维修单集中在哪个车间，针对性改进。用于质量风险管理。');
...
reg(58,7,2,'低分维修单分析','BD','总分低于阈值的维修单特征分析',execLowScore,'看低分维修单集中在哪个车间，分析共性原因。用于质量改进专项计划。');
...
reg(63,8,3,'班次 vs 故障类型','S,W','班次与故障原因交叉表及卡方检验',execShiftVsFault,'看某班次是否集中某类故障，可能与操作习惯相关。用于班次操作培训。');
...
/* ===== ⑨ 回归与预测 (5) ===== */
reg(66,9,3,'维修时长预测','P,L,S,AK','基于等级/设备/班次预测维修时长',execDurRegression,'看R²值，R²>0.3说明等级有一定预测力。用于维修工时预估和排程参考。');
reg(67,9,3,'维修成本预测','L,W,AD,AQ','基于设备/故障/分类预测维修成本',execCostRegression,'看回归方程斜率，斜率大说明时长对成本影响大。用于维修成本预估和审批参考。');
...
function execEquipHealth(rows) {
  ...
  const entries = Object.entries(groups).map(([equip, rs]) => {
    ...
    const score = freq * 0.4 + avgDur * 0.3 + avgCost * 0.3;
    return [equip, freq, avgDur.toFixed(2), avgCost.toFixed(2), score.toFixed(2)];
  }).sort((a, b) => parseFloat(b[4]) - parseFloat(a[4]));
  const tblRows = entries.slice(0, 30).map((e, i) => [i + 1, e[0], e[1], e[2], e[3], e[4]]);
  return '<p class="analysis-info">健康度评分 = 故障频率×0.4 + 平均维修时长×0.3 + 平均成本×0.3，分数越高越需关注</p>' + renderTable(['排名','设备','故障次数','平均维修时长','平均成本','健康度评分'], tblRows, 30) + exportCsvBtn('equip-health', ['排名','设备','故障次数','平均维修时长','平均成本','健康度评分'], tblRows);
}
...
function extractKeywords(text, minLen) {
  ...
  const words = cleaned.split(/\s+/).filter(w => w.length >= (minLen || 2));
  return words;
}
...
function execSeasonalDecomp(rows) {
  ...
  return chartCanvas('ac75') + renderTable(['月份', '实际值', '趋势', '季节性', '残差'], tblRows) +
    '<script>Charts.line("ac75",' + JSON.stringify(labels) + ',[{label:"实际值",data:' + JSON.stringify(actual) + '},{label:"趋势",data:' + JSON.stringify(trend.map(t => +t.toFixed(2))) + '},{label:"季节性",data:' + JSON.stringify(seasonalNorm.map((s, i) => i < actual.length ? +s.toFixed(2) : null)) + '},{label:"残差",data:' + JSON.stringify(residual.map(r => +r.toFixed(2))) + '}])</script>' +
...
function execKmeansCluster(rows) {
  ...
  const clusterNames = ['高频高耗', '中频中耗', '低频低耗', '特殊模式'];
...
reg(78,11,3,'季节性分解','Y','月度维修量趋势/季节性/残差三分解',execSeasonalDecomp,'看季节性指数，指数>1的月份是维修高峰期。用于季节性预防性维护计划制定。');
